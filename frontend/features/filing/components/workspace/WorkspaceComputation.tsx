  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <input
          disabled
          placeholder="Search heads, schedules or sections…"
          ...
          <Card key={s.id} id={`ws-${s.id}`} className="scroll-mt-24 overflow-hidden">
            ...
            {isOpen && (
              <div className="border-t border-ink-100 px-4 py-2">
                {s.entries.length === 0 ? (
                  <p className="py-2 text-sm text-ink-400">No entries captured — net computed as {formatInr(s.net)}.</p>
                ) : (
                  s.entries.map((e, i) => (
                ...
                <div className="mt-1 flex items-center justify-between border-t border-ink-100 pt-2 text-sm font-semibold">
                  <span className="text-ink-700">Net {s.title.replace(/^Income from /, '')}</span>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {[
          { label: 'Gross total income', value: gti, tone: 'text-ink-900' },
          { label: 'Total deductions', value: totalDeductions, tone: 'text-money-700' },
          { label: 'Taxable income', value: taxableIncome, tone: 'text-ink-900' },
          { label: `Tax payable (${regime})`, value: taxPayable, tone: 'text-payable-700' },
        ].map((k) => (
