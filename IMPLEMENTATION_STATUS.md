...
| **e-Return JSON** (`/api/v1/returns/{id}/itr-json:generate`, `:download`, `:validate`) | **Working** | All four individual forms (**ITR-1/2/3/4**) generate ITD-format JSON validated against the official NJsonSchema (conformance gate). ITR-4 (Sugam) emits a full **Schedule BP** — nature-of-business code, 44AD 6%/8% digital-vs-cash split, 44ADA 50%, per-vehicle 44AE goods carriage, and the no-account-case `FinanclPartclrOfBusiness`. ITR-3 emits Schedule BP (book↔tax depreciation), CG/DPM/DOA/DEP/UD, **Schedule IF** (partner-in-firm) + the s.10(2A) profit share in Schedule EI, plus FSI/TR1, FA, PTI, SPI, AMT. Headline figures anchored to the engine's `TaxComputation`. ~50 pre-upload business-rule checks. Live-verified: all 4 demo returns + a fresh create→compute→generate flow produce Valid/0-errors. |
